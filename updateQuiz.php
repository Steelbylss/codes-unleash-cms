...
<body>
    ...
    <nav>
        ...
        <a href="#" onclick="showSection('quizzes')" class="active">Quizzes</a>
        <a href="updateExam.php" onclick="showSection('exams')">Exams</a>
    </nav>
    <div class="content-container">
                <!-- Form for quizzes -->
                <div class="content section active" id="quizzesSection">
                <div class="dropdown" onclick="toggleDropdown()">
                <button class="dropbtn">Select Quiz Number</button>
                ...
                <div style="text-align: right; margin-right: 25px; margin-top: 20px;">
                    ...
                    <h2>Quiz</h2>
                    <form>
                        <label for="createquizRefNum" class="form-label">Reference Number:</label>
                        <input type="text" id="createquizRefNum" name="reference_number" class="form-input" readonly>

                        <label for="quizNum" class="form-label">Question Number :</label>
                        <input type="number" id="quizNum" name="question_number" class="form-input" required>
                        ...
                        <textarea id="questions" name="questions" class="form-input" rows="4"></textarea>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(event) {
                ...
                const referenceNumber = document.getElementById('createquizRefNum').value;
                const questionNumber = document.getElementById('quizNum').value;
                const question = document.getElementById('quizTitle').value;
                const codeSnippet = document.getElementById('questions').value;
                ...
                fetch(`http://192.168.1.7/api/chapter_assessment/update/${referenceNumber}`, {
                ...
                .then(response => {
                    ...
                    showAlert('Question updated successfully!'); // Show success message
                    window.location.reload(); // Reload the page to reflect the changes
                })
                .catch(error => {
                    console.error('Error updating question:', error);
                    showAlert('Failed to update question. Please try again.'); // Show error message
                });
            });
        });
        fetch('http://192.168.1.7/api/chapter_assessment', {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + token,
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(data => {
            ...
            data.results.forEach(chapter => {
                chapter.chapter_assessment.forEach(quiz => {
                    const option = document.createElement('a');
                    option.href = "#";
                    option.textContent = chapter.chapter + " - Question Number " + quiz.question_number;
                    option.onclick = function() {
                        // Set selected quiz in the dropdown button
                        document.querySelector('.dropbtn').textContent = quiz.chapter + " - Question Number " + quiz.question_number;
                        // Populate form fields with selected quiz details
                        document.getElementById('createquizRefNum').value = quiz.reference_number;
                        document.getElementById('quizNum').value = quiz.question_number;
                        document.getElementById('quizTitle').value = quiz.question;
                        document.getElementById('questions').value = quiz.code_snippet || '';
                        document.getElementById('choice1').value = quiz.choice_1;
                        document.getElementById('choice2').value = quiz.choice_2;
                        document.getElementById('choice3').value = quiz.choice_3;
                        document.getElementById('choice4').value = quiz.choice_4;
                        document.getElementById('answer').value = quiz.correct_answer;
                        toggleDropdown();
                    };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButton.addEventListener('click', function(event) {
                ...
                fetch(`http://192.168.1.7/api/chapter_assessment/delete/question/${referenceNumber}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': 'Bearer ' + token,
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                .catch(error => {
                    ...
                    showAlert('Failed to delete question. Please try again.'); // Show error message
                });
            });
        });
