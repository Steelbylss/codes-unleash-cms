...
<body>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('http://192.168.1.7/api/user', {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                ...
                data.forEach(user => {
                    const emailVerifiedTimestamp = user.email_verified_at ? new Date(user.email_verified_at).toLocaleString() : 'Not Verified';
                    const row = document.createElement('tr');
                    row.innerHTML = `
                        <td>${user.id}</td>
                        <td>${user.username}</td>
                        <td>${user.email}</td>
                        <td>${emailVerifiedTimestamp}</td>
                        <td>${user.otp}</td>
